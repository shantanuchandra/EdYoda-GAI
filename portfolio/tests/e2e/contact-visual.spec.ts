...
test("Contact uses the reference panel composition without introducing a form", async ({ page }) => {
  ...
  const geometry = await page.evaluate(() => {
    ...
    return { canvas: box(canvas), heading: { ...box(heading), fontSize: Number.parseFloat(getComputedStyle(heading).fontSize), textAlign: getComputedStyle(heading).textAlign }, description: box(description), panels: panels.map((panel) => ({ ...box(panel), top: panel.getBoundingClientRect().top })), overflow: document.documentElement.scrollWidth - innerWidth };
  });
